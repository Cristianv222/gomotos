...
class Venta(models.Model):
    ...
    def get_ventas_por_dia(fecha=None):
        ...
        total_servicios = sum(
            detalle.total for venta in ventas 
            for detalle in venta.detalleventa_set.filter(es_servicio=True)
        )
...
class DetalleVenta(models.Model):
    ...
    # Campos de servicio simplificados
    nombre_personalizado = models.CharField(
        max_length=200,
        blank=True,
        null=True,
        help_text="Nombre para items manuales, servicios o genéricos"
    )
    ...
    def __str__(self):
        if self.producto:
            ...
            return f"{self.nombre_personalizado} - {self.cantidad} x ${self.precio_unitario}"
